 ...
 * Page alter.
 */
function excelpointb_page_alter($page) {
 ...
 */
function excelpointb_preprocess_html(&$variables) {
  /* Color */
  ...
  drupal_add_css(path_to_theme() . '/css/'. $file, array('group' => CSS_THEME, 'weight' => 115,'browsers' => array(), 'preprocess' => FALSE));
	...
	*/
	if (theme_get_setting('bootstrap_js_include', 'excelpointb')) {
	drupal_add_js(drupal_get_path('theme', 'excelpointb') . '/bootstrap/js/bootstrap.min.js');
	}
	//EOF:Javascript
	...
	*/
	if (theme_get_setting('scrolltop_display', 'excelpointb')) {
 ...
 */
function excelpointb_preprocess_page(&$vars) {
 ...
 */
function excelpointb_process_page(&$variables) {
  // Hook into color.module.
  if (module_exists('color')) {
 ...
 */
function excelpointb_preprocess_block(&$variables) {
